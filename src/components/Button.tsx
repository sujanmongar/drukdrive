...
const variants: Record<Variant, string> = {
  ...
  // Standalone text action (Edit, Cancel, Clear all): no box, 44px target.
  link: "",
};

const sizes: Record<Size, string> = {
  sm: "text-xs px-3 py-2 min-h-11 lg:min-h-0",
...
export default function Button(props: ButtonProps | LinkProps) {
  ...
  const classes =
    variant === "link"
      ? `${actionLink} disabled:cursor-not-allowed disabled:opacity-50 ${className}`
