...
export function FieldButton({
  ...
  return (
    ...
    >
      <Icon name={icon} size={20} className="shrink-0 text-[color:var(--color-ink)]" />
      <span className="flex flex-col gap-1 leading-none">
        <span className="text-[11px] text-[color:var(--color-ink-soft)]">{label}</span>
        <span className="text-sm font-bold text-[color:var(--color-ink)]">{value}</span>
      </span>
    </button>
  );
}
  ...
  return (
    <label className="flex w-full flex-col gap-1.5">
      ...
      <span className="flex h-[52px] items-center gap-2 rounded-xl border border-[color:var(--color-border)] px-3.5 focus-within:border-[color:var(--color-ink)]">
        ...
        <input
          className="w-full bg-transparent text-sm text-[color:var(--color-ink)] outline-none placeholder:text-[color:var(--color-muted)]"
          {...props}
        />
      </span>
    </label>
