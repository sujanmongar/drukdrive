...
export default function LoginSecurityEditor() {
  ...
  const labelClass = "mb-1.5 block t-body-sm font-semibold text-[color:var(--color-ink)]";
  ...
  return (
    <div className="mt-6 flex flex-col gap-6">
      <section className="rounded-2xl border border-[color:var(--color-border)] bg-white p-5 shadow-card sm:p-6">
        ...
        <p className="mt-1 t-body-sm">Use at least 8 characters. You&rsquo;ll stay signed in on this device.</p>
        <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <label>
            ...
            {confirm && next !== confirm && <p className="mt-1 t-caption text-[color:var(--color-danger)]">Passwords don&rsquo;t match.</p>}
          </label>
        </div>
      ...
      <section className="rounded-2xl border border-[color:var(--color-border)] bg-white p-5 shadow-card sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="t-h4">Two-step verification</h3>
            <p className="mt-1 t-body-sm">A one-time code is sent to {user.phone} whenever you sign in on a new device.</p>
          </div>
          <button
            type="button"
            role="switch"
          ...
          >
            <span className={`absolute top-1 size-5 rounded-full bg-white shadow-card transition-transform ${twoFactor ? "translate-x-6" : "translate-x-1"}`} />
          </button>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-[color:var(--color-border)] bg-white p-5 shadow-card sm:p-6">
        ...
        <ul className="mt-4 divide-y divide-[color:var(--color-border)]">
          {[
            { device: "iPhone · Safari", place: "Thimphu, Bhutan", when: "This device" },
            { device: "MacBook · Chrome", place: "Paro, Bhutan", when: "2 days ago" },
          ].map((s) => (
            <li key={s.device} className="flex items-center justify-between gap-4 py-3">
              <div>
                <p className="t-body font-semibold text-[color:var(--color-ink)]">{s.device}</p>
                <p className="t-caption">
                  {s.place} · {s.when}
                </p>
              </div>
              {s.when !== "This device" && (
                <button type="button" className="min-h-11 rounded-lg px-3 t-body-sm font-semibold text-[color:var(--color-danger)] hover:bg-[color:var(--color-danger-bg)]">
                  Sign out
                </button>
              )}
            </li>
